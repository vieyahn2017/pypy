...
struct vsc_host_mgr {
    ...
    __u32 active_host_count;    /*活动host个数*/
    struct vsc_ctrl_info* host_list[MAX_HOST_NR_PER_VBS];
};

#define VSC_DATA_MAGIC            0x615461442d437356     /* VsC-DaTa */
#define VSC_DATA_VER_MAJOR        (1)                    /* 数据主版本号，主版本号数据做升级、回退能力*/ 
#define VSC_DATA_VER_MINOR        (1)                    /* 数据次要版本号，次要版本号，做数据兼容，前向数据兼容。*/

struct vsc_scsi_data_head {
    unsigned long                 magic;                 /* 魔数 */
    unsigned int                  ver_major;             /* 版本号 */
    unsigned int                  ver_minor;             /* 次要版本号 */
    unsigned long                 size;                  /* 数据长度 */
    unsigned long                 reserved[32];          /* 保留字段 */
} __attribute__ ((packed));

struct vsc_scsi_data_struct {
    struct vsc_scsi_data_head     head;                  /* 数据头 */
    __u64                         cmd_sn;                /* 命令序列号 */
    __u64                         event_sn;              /* 事件序列号 */
    unsigned int                  suspend;               /* 数据发送挂起 */
    int                           vbs_host_id; 
    struct vsc_ctrl_statics       stat;                  /* 统计数据 */
} __attribute__ ((packed));

#define VSC_SCSI_DEVICE_RESERVE_SIZE    (2 * 1024)
/* 该数据结构的大小不能超过 VSC_SCSI_DEVICE_RESERVE_SIZE
 * 热补丁涉及到这块数据的变更必须保持这些数据及操作的兼容*/
...
/**************************************************************************
 功能描述  : 退出所有event事件
 参    数  : struct vsc_ctrl_info *h  host控制句柄
...
/**************************************************************************
 ...
 返 回 值  : struct vsc_ctrl_info *         host控制句柄
**************************************************************************/
...
static int vsc_eh_device_reset_handler(struct scsi_cmnd *sc)
{
    ...
    /* <liaodf 20151225>现在device reset实际上相当于host reset，把该vbs上所有io都释放掉，
       修改逻辑为device reset，只处理入参io同一个卷上的io */
    if (SUCCESS == e->event_priv) {
        /* 所有命令重新入队，释放对应的资源 */
...
/**************************************************************************
 功能描述  : 控制结构释放函数，引用=0时释放内存
 参    数  : 
 返 回 值  : vsc_ctrl_info *h:     host控制句柄
**************************************************************************/
...
struct vsc_ctrl_info *vsc_register_host( struct vsc_ioctl_cmd_add_host *host_info )
{
    ...
    /* 注册scs host，并申请存放vsc_ctrl_info的内存 */
    ...
    /* 注册scsi host，并申请存放vsc_ctrl_info的内存 */
...
int vsc_remove_host(struct vsc_ctrl_info *h)
{
    ...
    /* 如果host所属host mgr上已经没有剩余host，释放host mgr */
    if (mgr->host_count == 0)
    {
        list_del(&mgr->node);
        kfree(mgr);
...
int vsc_load_host_data(struct vsc_ctrl_info *h, void *data, int size)
{
    ...
    if (scsi_head->ver_major == VSC_DATA_VER_MAJOR) {
        ...
        if (scsi_head->ver_minor > VSC_DATA_VER_MINOR) {
            ...
            /*  如果原始数据版本比较新，但存储的数据大小小于当前大小，则认为数据无效 */
            ...
            /*  如果原始数据版本比较旧，但存储的数据大小大于当前大小，则认为数据无效 */
            if (scsi_head->size > sizeof(struct vsc_scsi_data_struct)) {
                vsc_err("load host data failed, host data is invalid, length is invald, %lu to %lu\n",
                        scsi_head->size, sizeof(struct vsc_scsi_data_struct));
                return 0;
...
static int __init vsc_init( void )
{
     ...
     */
     /* sizeof(__u32)*2: 为buffer增加头尾magic校验，提前发现踩内存的问题 */
